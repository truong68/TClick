...
try:
    ...
    # Sau khi tìm thấy trang https://duhocvietphuong.edu.vn, thực hiện cuộn trang mượt mà trong 300 giây
    if driver.current_url == "https://duhocvietphuong.edu.vn/":
        print("Đang cuộn trang lên xuống mượt mà trong 300 giây...")
        start_time = time.time()

        while time.time() - start_time < 300:  # Giữ trang mở trong 300 giây (5 phút)
            # Cuộn trang xuống dưới với hiệu ứng mượt mà
            driver.execute_script("window.scrollTo({top: document.body.scrollHeight, behavior: 'smooth'});")
            time.sleep(2)  # Chờ một chút trước khi cuộn lên
            # Cuộn trang lên trên với hiệu ứng mượt mà
            driver.execute_script("window.scrollTo({top: 0, behavior: 'smooth'});")
            time.sleep(2)  # Chờ một chút trước khi cuộn xuống tiếp
